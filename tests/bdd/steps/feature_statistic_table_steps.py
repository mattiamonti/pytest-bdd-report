...
@then(
    parsers.cfparse(
        "the feature statistic table should have a row for the feature '{feature_name}'"
    )
)
...
@then(
    parsers.cfparse(
        "the feature statistic table row for the feature '{feature_name}' should have {value} in column '{column_name}'"
    )
)
def has_column(page: Page, feature_name: str, value: str, column_name: str):
    ...
    if column_name == "Duration":
        match value:
            case ">0":
                assert table.get_column_duration() > 0.0
            case "=0":
                assert table.get_column_duration() == 0.0
    else:
