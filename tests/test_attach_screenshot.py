...
def test_attach_screenshot_from_bytes_is_saved(
    mock_bytes, feature_name, scenario_name, teardown
):
    ...
    assert screenshot_repo.get(feature_name, scenario_name).encoded_image != ""


def test_encoding_of_screenshot_from_bytes(
    mock_bytes, feature_name, scenario_name, teardown
):
    attach.screenshot(mock_bytes, feature_name, scenario_name)
    image_base64 = screenshot_repo.get(feature_name, scenario_name).encoded_image
...
def test_retrieving_screenshot_attached_from_bytes(
    mock_bytes, feature_name, scenario_name, teardown
):
    attach.screenshot(mock_bytes, feature_name, scenario_name)
...
def test_retrieving_screenshot_with_multiple_saved_with_same_feature(
    mock_bytes, feature_name, scenario_name, teardown
):
    ...
    first_image = screenshot_repo.get(feature_name, scenario_name).encoded_image
    second_image = screenshot_repo.get(feature_name, "scenario 2").encoded_image
...
def test_add_more_screenshot_with_same_feature_and_different_scenario(
    mock_bytes, feature_name, scenario_name, teardown
):
    attach.screenshot(mock_bytes, feature_name, scenario_name)
...
def test_add_more_screenshot_with_same_scenario_and_different_feature(
    mock_bytes, feature_name, scenario_name, teardown
):
    attach.screenshot(mock_bytes, feature_name, scenario_name)
