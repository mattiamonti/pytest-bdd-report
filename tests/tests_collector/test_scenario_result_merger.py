import pytest
from collector.scenario_result_merger import ScenarioAndResultMerger, LengthMismatchError
...
def test_merge_scenario_and_result_length(mock_aggregated_scenarios, mock_tests_results):
...
def test_merge_scenario_and_result_content(mock_aggregated_scenarios, mock_tests_results):
    merger = ScenarioAndResultMerger()
    merged_results = merger.merge_scenario_and_result(
        mock_aggregated_scenarios, mock_tests_results)
...
def test_merge_scenario_and_result_mismatch(mock_aggregated_scenarios):
    ...
    with pytest.raises(LengthMismatchError):
        merger.merge_scenario_and_result(
            mock_aggregated_scenarios, mock_tests_results)
